"""Enhanced chatbot core with Ollama and transformers support."""
...
try:
    ...
    print("⚠️  Transformers not available - will use Ollama only")
...
class LocalLLM:
    """Local language model wrapper with transformers fallback."""
    ...
    def __init__(self):
        ...
        if OLLAMA_AVAILABLE:
            ...
            print("⚠️  No LLM backend available - using rule-based responses only")
    ...
    def _initialize_ollama(self):
        ...
        try:
            ...
            if models:
                ...
                # Use the first available model or llama2
                if models:
                    self.ollama_client.model = models[0].split(':')[0]
                    print(f"Using Ollama model: {self.ollama_client.model}")
            else:
    ...
    def generate_response(self, prompt: str, context: str = "") -> str:
        ...
        if self.ollama_client:
            return self.ollama_client.generate_response(prompt, context)
        elif self.pipeline:
    ...
    def _generate_transformers_response(self, prompt: str, context: str = "") -> str:
        ...
        try:
            ...
            print(f"Error generating transformers response: {e}")
            return self._generate_fallback_response(prompt)
    
    def _format_prompt(self, user_input: str, context: str = "") -> str:
        """Format the prompt for transformers model."""
        if context:
...
class ChatbotCore:
    """Enhanced chatbot core with Ollama support."""
    ...
    def process_query(self, user_input: str, external_context: Optional[str] = None) -> ChatResponse:
        ...
        if self.llm.ollama_client:
            sources.append("ollama")
        elif self.llm.pipeline:
        ...
        response = ChatResponse(
            ...
            metadata={
                ...
                "llm_backend": "ollama" if self.llm.ollama_client else "transformers" if self.llm.pipeline else "fallback"
            }
        )
    ...
    def reset_conversation(self):
        ...
        if self.llm.ollama_client:
            self.llm.ollama_client.reset_conversation()
    ...
    def get_conversation_summary(self) -> Dict[str, Any]:
        ...
        if self.llm.ollama_client:
            summary["llm_backend"] = "ollama"
            summary["ollama_model"] = self.llm.ollama_client.model
        elif self.llm.pipeline:
            summary["llm_backend"] = "transformers"
            summary["transformers_model"] = self.llm.model_name
        else:
            summary["llm_backend"] = "fallback"
